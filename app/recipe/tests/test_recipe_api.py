...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe2 = sample_recipe(user=self.user, title='paneer tikka')
        tag1 = sample_tag(user=self.user, name='Non-vegetarian')
        tag2 = sample_tag(user=self.user, name='vegetarian')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title='Mutton korma')
        res = self.client.get(
            RECIPES_URL,
